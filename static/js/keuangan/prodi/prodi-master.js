...
const baseUrl = "https://hris_backend.ulbi.ac.id/api/v2/prodi";
...
function createRow(item) {
  return `<tr>
      <td>${item.nama}</td>
      <td>${item.bimbingan.pb1}</td>
      <td>${item.bimbingan.nominalPB1}</td>
      <td>${item.bimbingan.pb2}</td>
      <td>${item.bimbingan.nominalPB2}</td>
      <td>${item.menguji.penguji_utama}</td>
      <td>${item.menguji.nominalPU}</td>
      <td>${item.menguji.penguji_pendamping}</td>
      <td>${item.menguji.nominalPP}</td>
...
function fetchAllPages(page) {
  const url = `https://hris_backend.ulbi.ac.id/api/v2/prodi/get?page=${page}`;
  fetch(url, {
    method: "GET",
    headers: {
      login: `${token}`,
...
function sendDeleteRequest(id) {
  const url = `https://hris_backend.ulbi.ac.id/api/v2/prodi/delete?id=${id}`;
  fetch(url, {
    method: "DELETE",
    headers: {
...
function DeleteHonor() {
  const url = "https://hris_backend.ulbi.ac.id/api/v2/prodi/deleteall";
