...
const baseUrl = "https://hris_backend.ulbi.ac.id/api/v2/master/pph";
...
function createRow(item) {
  ...
  return `<tr>
    ...
    <td>${item.bulan}</td>
    <td>${item.tahun}</td>
    <td>${item.persentase_pph}</td>        
    <td>
        <button class="btn btn-primary btn-sm edit-btn" data-id="${item.idpph}" data-id="${item.idpph}" onclick="editItem(this)">
            <i class="mdi mdi-table-edit"></i>
        </button>
        <button class="btn btn-danger btn-sm delete-btn" data-id="${item.idpph}" data-id="${item.idpph}" onclick="deleteItem(this)">
...
window.editItem = function (element) {
  ...
  window.location.href = "pph-edit.html";
};
...
function sendDeleteRequest(id) {
  const url = `https://hris_backend.ulbi.ac.id/api/v2/master/pph/byid?id=${id}`;
  fetch(url, {
    method: "DELETE",
    headers: {
...
function postData() {
  ...
  const persentase_pph = document.getElementById("persentase_pph");
