...
function groupCoursesByInstructor(data) {
  ...
  data.forEach((row) => {
    const key = row["Nama"] + "_" + row["Angkatan"] + "_" + row["Kelas"]
    if (!instructors[key]) {
      instructors[key] = {
        nama: row["Nama"],
        masa_perolehan: row["Masa Perolehan"],
        persentase_pph: parseFloat(row["Persentase PPH"]),

        kelas: [],
      };
    }
    instructors[key].kelas.push({
        angkatan: row["Angkatan"],
        prodi: row["Program Studi"],
        semester: row["Semester"],
        kelas: row["Kelas"],
        tahapan: {
          tahap1: row["Laporan Perwalian"],
          tahap2: row["Laporan Perwalian"],
        },
        honor: parseFloat(
          row["Honor"]
        )
...
function postInstructorData(data) {
  const url =
    "https://hris_backend.ulbi.ac.id/api/v2/dosenwali/insert";
  fetch(url, {
    method: "POST",
    headers: {
...
function downloadExcelTemplate() {
  ...
  XLSX.writeFile(wb, "DosenWali_Template.xlsx");
